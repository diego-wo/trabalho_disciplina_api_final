...
async def gerar_historia_evolutiva(
    ...
    """
    Gera uma história evolutiva a partir de um tema, utilizando duas LLMs em cadeia para refinar a narrativa.

    Args:
        tema (str): Tema da história.
        estilo (str): Estilo desejado para a história (default: "aventura").
        genero (str): Gênero da história (default: "ficção").
        extensao (str): Extensão desejada para a história (default: "media").
        x_api_token (str): Token de autenticação.

    Returns:
        dict: Dicionário contendo o esboço, a história refinada, tempos de execução e informações de token da OpenAI.
    """
    ...
    try:
        ...
        # Converte os dados de tokens para um formato serializável
        tokens_refinado_raw = response_refinado.usage.__dict__
        tokens_refinado: Dict[str, Any] = {
            k: (v if isinstance(v, (int, float, str, bool, list, dict)) else str(v))
            for k, v in tokens_refinado_raw.items()
        }
...
async def resumir_texto_avancado(
    ...
    Args:
        ...
        tamanho (str): Customização da extensão do resumo (default: "conciso").
        x_api_token (str): Token de autenticação.

    Returns:
        dict: Dicionário contendo o resumo, tempo de execução e dados de token.
    """
